...
class _MethodAppState extends State<MethodApp> {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        setState(() {
          ...
          _status = 'Opened via a VERIFIED Android App Link — no chooser shown';
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Method Demo: Verified App Links',
      theme: ThemeData(colorSchemeSeed: const Color(0xFF6A4C93), useMaterial3: true),
      home: Scaffold(
        appBar: AppBar(title: const Text('Verified Android App Links')),
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              const Text(
                'This app demonstrates Android App Links verification '
                '(autoVerify="true" + a hosted assetlinks.json).',
                style: TextStyle(fontSize: 15, color: Colors.black54),
              ),
              const SizedBox(height: 8),
              const Text(
                'Because the domain is proven to belong to this app alone, '
                'tapping a https://…/methods/applinks/:token link opens this '
                'app DIRECTLY — Android never shows the "Open with…" chooser, '
                'even if other apps are installed.',
